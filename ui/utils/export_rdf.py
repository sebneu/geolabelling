...
def addMetadata( obj, graph, location_search):
    ...
    if 'metadata_temp_start' in obj:
        graph.add((dist, TIMEX.hasStartTime, Literal(obj['metadata_temp_start'] + 'T00:00:00', datatype=XSD.dateTime)))
    if 'metadata_temp_end' in obj:
        graph.add((dist, TIMEX.hasEndTime, Literal(obj['metadata_temp_end'] + 'T00:00:00', datatype=XSD.dateTime)))

    if 'data_temp_start' in obj:
        graph.add((ref, TIMEX.hasStartTime, Literal(obj['data_temp_start'] + 'T00:00:00', datatype=XSD.dateTime)))
    if 'data_temp_end' in obj:
        graph.add((ref, TIMEX.hasEndTime, Literal(obj['data_temp_end'] + 'T00:00:00', datatype=XSD.dateTime)))

    if 'transaction_time' in obj:
        graph.add((ref, TIMEX.transactionTime, Literal(obj['transaction_time'] + 'T00:00:00', datatype=XSD.dateTime)))
            ...
            if 'entities' in c or 'dates' in c:
                for row_i, v in enumerate(c['values']['exact']):
                    ...
                    if 'dates' in c and c['dates'][row_i]:
                        graph.add((cell, CSVWX.hasTime, Literal(c['dates'][row_i] + 'T00:00:00', datatype=XSD.dateTime)))
